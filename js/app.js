...
const state = { base: 'All', based: 'All', generation: 'All', device: 'All', life: 'All', license: 'All', family: 'All', installs: 'All', kind: 'All', author: 'All', query: '', yearMin: YEAR_MIN, yearMax: YEAR_MAX };
...
const index = osData.map((os) => ({
  os,
  text: [os.name, os.knownFor, os.pitch ?? '', String(os.released), os.baseOS, baseLabel(os.baseOS), os.distroBase, unixStatus[os.unix].label, generations[os.generation]?.label ?? '', lifecycle[lifeKey(os)].label, os.license, ...os.by, ...os.basedOn, ...os.devices, ...SPECS.flatMap(([k]) => os.specs[k])].join('\n').toLowerCase(),
...
function openModal(id) {
  ...
  const facts = [
    ...
    ['Base', chip(`${uiIcon('calendar', 'w-3.5 h-3.5')}${os.released}`, 'Year of creation') +
      chip(esc(baseLabel(os.baseOS) + (os.distroBase ? ` · ${os.distroBase}` : ''))) +
